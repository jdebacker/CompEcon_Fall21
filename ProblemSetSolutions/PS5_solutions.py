...
def create_x(merger_df, id_array):
    ...
    buyer_chars = merger_df[['buyer_id', 'year', 'buyer_lat', 'buyer_long', 'num_stations_buyer', 'corp_owner_buyer']].copy()
    ...
    target_chars = merger_df[['year', 'target_id', 'target_lat', 'target_long', 'hhi_target', 'population_target', 'price']].copy()
    # there should not be repeated targets, but drop duplicates to be safe
    target_chars.drop_duplicates(subset=['target_id', 'year'], keep='first',
                                inplace=True)

    # put buyer and target ids in a dataframe to enable merge with X's
    df = pd.DataFrame(id_array, index=range(id_array.shape[0]), columns=['year', 'buyer_id', 'target_id'])

    # Merge buyer and target characteristics onto dataframes with buyer and target ids
    df = df.merge(buyer_chars, how='left', left_on=['year', 'buyer_id'],
                  right_on=['year', 'buyer_id'], copy=True)
    df = df.merge(target_chars, how='left', left_on=['year', 'target_id'],
                  right_on=['year', 'target_id'], copy=True)
    ...
    df['distance'] = df.apply(lambda row: distance(row['buyer_loc'], row['target_loc']).miles, axis=1)
...
def Qscore(coeffs, data_dict, covariates, use_price, smoothed_estimator):
    ...
    if smoothed_estimator:
        if use_price:
            ...
            value = ((payoff(coeffs, data_dict['bt'], covariates) +
                     payoff(coeffs, data_dict['bptp'], covariates)) -
                     (payoff(coeffs, data_dict['btp'], covariates) +
                     payoff(coeffs, data_dict['bpt'], covariates)))
            ineq = norm.cdf(value.astype(float), scale=1/30)
        ...
        if use_price:
            coeffs = np.append(coeffs, -1.0)   # price in last column
            ineq = ((payoff(coeffs, data_dict['bt'], covariates) >=
                     payoff(coeffs, data_dict['btp'], covariates)) &
                    (payoff(coeffs, data_dict['bptp'], covariates) >=
                     payoff(coeffs, data_dict['bpt'], covariates)))
        else:
...
def estimate_mse(init_params, covariates, data_dict, use_price=True,
                 smoothed_estimator=False, method="NM",
                 print_results=False):
    '''
    ...
    # Nelder-Mead method
    if method == "NM":
        results = opt.minimize(Qscore, init_params, method='Nelder-Mead',
                                args=(data_dict, covariates, use_price,
                                      smoothed_estimator),
                                tol=1e-13)
    # Differential evolution method
    elif method == "DE":
        bnds = [(-20000, 20000)] * (len(covariates) - 1)
        results = opt.differential_evolution(Qscore, bnds,
                                              args=(data_dict, covariates, use_price,
                                      smoothed_estimator),
                                              strategy='best1bin',
                                              maxiter=1000,
    ...
    # Simulated annealing method
        ...
        results = opt.basinhopping(Qscore, init_params, niter=1000, T=1.0,
                                    stepsize=0.05,
                                    minimizer_kwargs={"args":
                                                      (data_dict, covariates, use_price,
                                      smoothed_estimator)},
                                    interval=50)
    else:
        print('Please enter a valid optimization method - or nothing'
...
def merger_estimate(init_params, covariates, data, use_price=False, smoothed_estimator=False, method='DE'):
    '''
    ...
    results = estimate_mse(init_params, covariates, data_dict, use_price,
                 smoothed_estimator, method)
...
results = merger_estimate(
    init_guesses, covariate_list1, radio_data,
                                    use_price=False,
...
init_guesses = np.array([-0.002, 7.54, 2.28, -0.18])  # last coeff on price normalized to -1
results = merger_estimate(
    init_guesses, covariate_list2, radio_data,
                                    use_price=True,
                                    smoothed_estimator=False,
                                    method='SA')
mse_results.append(results)
...
out_dict = {'Variable': covariate_list2 + ["Score"],
            'Model 1': [], 'Model 2': []}
for i, v in enumerate(mse_results):
    out_dict['Model ' + str(i + 1)].extend(v['x'])
    if len(out_dict['Model ' + str(i + 1)]) < len(covariate_list2):
        out_dict['Model ' + str(i + 1)].extend(["-"] * (len(covariate_list2) - len(out_dict['Model ' + str(i + 1)])))
